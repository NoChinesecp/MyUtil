...
//    在启动类App.class文件中配置Bean来设置文件大小
    ...
    public String downloadFile(HttpServletRequest request, HttpServletResponse response) {
       ...
       String fullpath= path+"static/Upload/";
        ...
        String[] fileName =request.getParameterValues("download");
        File file =null;
        for (int i = 0; i <fileName.length ; i++) {
            if (fileName[i] != null) {
                //设置文件路径
                String realPath = fullpath;
                file = new File(realPath , fileName[i]);
                if (file.exists()) {
                    // 设置强制下载不打开
                    ...
                    try {
                        ...
                        int kb = bis.read(buffer);
                        while (kb != -1) {
                            os.write(buffer, 0, kb);
                            kb = bis.read(buffer);
                        }
